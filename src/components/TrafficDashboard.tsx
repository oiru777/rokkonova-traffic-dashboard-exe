...
const TrafficDashboard: React.FC<TrafficDashboardProps> = ({ selectedDate, csvData, isUsingCsv }) => {
  const { data, loading, error } = useTrafficData(selectedDate, csvData, isUsingCsv);

  if (loading) return <LoadingSpinner />;
  if (error) return <ErrorMessage message={`交通データの取得に失敗しました: ${error}`} />;
  if (!data || data.length === 0) {
    return <ErrorMessage message="選択した日付の交通データがありません" />;
  }
  ...
  const usageTypeData = data.reduce((acc, item) => {
    acc[item.usage_type] = (acc[item.usage_type] || 0) + item.vehicle_count;
    return acc;
  }, {} as Record<string, number>);
